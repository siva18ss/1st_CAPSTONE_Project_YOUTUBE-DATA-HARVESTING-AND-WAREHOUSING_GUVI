...
#PAGE CONFIGURATION
icon = Image.open(r"C:\Users\DivDin\Documents\MyGitHub\you-tube-logo-hd-png-download.png")
st.set_page_config(page_title= "Youtube Data Harvesting and Warehousing Project",
                   page_icon= icon,
                   layout= "wide",
                   initial_sidebar_state= "expanded",
                   menu_items={'About': """Application created by Sivasubramaniyan N!*"""})

#API key connection
def Api_connect():
    Api_Id="AIzaSyBx_xB0Ykmd83sUjKNPvRloZeJP21wsWmo"
...
# def channel_names():   
#     ch_name = []
#     for i in db["channel details"].find({},{"_id":0,"channel_information":1}):   
...
show_table = st.radio("SELECT TABLE",(":black[Channels]",":green[Playlists]",":yellow[Videos]",":orange[Comments]"))

if show_table == ":black[Channels]":
    show_channels_table()
elif show_table == ":green[Playlists]":
    show_playlists_table()
elif show_table ==":yellow[Videos]":
    show_videos_table()
...
question = st.selectbox(
    ...
    ('1. All the videos and the Channel Name',
     ...
     '8. What are all the videos published in the year 2022 by corresponding channel?',
     '9. Average duration of all videos in each channel',
     '10. Videos with highest number of comments'))

     
if question == '1. All the videos and the Channel Name':
    query1 = '''SELECT title AS Video_Title, channel_name AS Channel_Name FROM videos ORDER BY channel_name;'''
    cursor.execute(query1)
    df = pd.DataFrame(cursor.fetchall(),columns=["Video Title","Channel Name"])
    st.write(df)
    mydb.commit()

elif question == '2. Which channels have the most number of videos?':
    query2 = """SELECT channel_name AS Channel_Name, total_videos AS Total_Videos FROM channels ORDER BY total_videos DESC"""
    cursor.execute(query2)
    df = pd.DataFrame(cursor.fetchall(),columns=["Channel Name","No Of Videos"])
    mydb.commit()
    st.write(df)
    ...
    df = pd.DataFrame(cursor.fetchall(),columns=["views","channel Name","video title"])
    mydb.commit()
    st.write(df)
    ...
    df = pd.DataFrame(cursor.fetchall(),columns=["No Of Comments", "Video Title"])
    mydb.commit()
    st.write(df)
...
elif question == '6. What is the total number of likes and dislikes for each video?':
    query6 = '''select Likes as likeCount,Title as VideoTitle from videos;'''
    cursor.execute(query6)
    df = pd.DataFrame(cursor.fetchall(),columns=["like count","video title"])
    mydb.commit()
    st.write(df)
...
elif question == '8. What are all the videos published in the year 2022 by corresponding channel?':
    query8 = '''select Title as Video_Title, Published_Date as VideoRelease, Channel_Name as ChannelName from videos 
                where extract(year from Published_Date) = 2022;'''
    cursor.execute(query8)
    df = pd.DataFrame(cursor.fetchall(),columns=["Name", "Video Publised On", "ChannelName"])
    mydb.commit()
    st.write(df)

elif question == '9. Average duration of all videos in each channel':
    # query9 = '''SELECT channel_name, SUM(duration_sec) / COUNT(*) AS average_duration FROM (SELECT channel_name, 
    #                         CASE
    ...
    #                             END AS duration_sec
    #                     FROM videos
    #                     ) AS subquery GROUP BY channel_name'''
    query9 =  "SELECT Channel_Name as ChannelName, AVG(Duration) AS average_duration FROM videos GROUP BY Channel_Name;"
    cursor.execute(query9)
    df = pd.DataFrame(cursor.fetchall(),columns=['ChannelTitle', 'Average Duration'])
    mydb.commit()
    T9=[]
    for index, row in df.iterrows():
